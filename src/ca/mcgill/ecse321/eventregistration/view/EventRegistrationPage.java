...
public class EventRegistrationPage extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -207672715174813808L;
	/**
	 * 
	 */
	//private static final long serialVersionUID = -5263750109451453265L;
	...
	private void initComponents(){
		...
		participantLabel.setText("Select Partcicpant:");
		...
		startTimeLabel.setText("Satrt Time:");
		...
		layout.setHorizontalGroup(
				...
				.addGroup(layout.createSequentialGroup()
						...
						.addGroup(layout.createParallelGroup()
								...
								.addComponent(addEventButton))) // TODO:  is there a ) missing?
						);
	...
	private void refreshData(){
		...
		if(error == null || error.length()==0){
			...
			Iterator<Participant> pIt = rm.getParticipants().iterator(); //TODO: may need to change the iterator class
			Integer index = 0;
			while(pIt.hasNext()){
		...
		//this is needed because the size of the window changes depending in whether an error message is shown or not
		pack();
	}
	...
	private void addEventButtonActionPerformed(java.awt.event.ActionEvent evt){
		...
		//force the same date for start and end end times to  ensure  that only the times differ
	...
	private void registerButtonActionPerformed(java.awt.event.ActionEvent evt){
		...
		if(selectedParticipant<0)
			error = error +"Partcicpant needs to be selected for registration!";
		if(selectedEvent<0)
